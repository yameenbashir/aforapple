 ...
 */
@Controller
@RequestMapping("/salesReportWithSale")
public class SalesReportWithSaleController {
			...
			try {
				...
				if(reportType.equals("Revenue")){
					...
					if(!isHeadOffice && configuration!=null && configuration.getPropertyValue().toString().equalsIgnoreCase(ControllersConstants.TRUE)){
						...
						reportParams.setBaseColumn("sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,sum(Gross_Profit) as Profit,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
						reportParams.setPrintColumns( reportType+","+ "Cost of Goods,Gross Profit,Margin,Tax,Avg Discount");
					}
					...
					reportParams.setTableName("SaleReport_withSale");
					...
					ReportParams reportParams = new ReportParams();
					reportParams.setBaseColumn("sum(Cost_of_Goods) as Cost,sum(Revenue) as Revenue,sum(Gross_Profit) as Profit,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
					reportParams.setCompanyId(currentUser.getCompany().getCompanyId());
					reportParams.setEndDate(endDat);
					reportParams.setGroupBy("Product");
					reportParams.setMainBaseColumn("Product");
					reportParams.setOrderBy("");
					reportParams.setPivotColumn( "CREATED_DATE");
					reportParams.setPrintColumns( reportType+","+"Revenue,Gross Profit,Margin,Tax,Avg Discount");
					reportParams.setReportDateType(reportDateType);
					reportParams.setStartDate(startDat);
					reportParams.setTableName("SaleReport_withSale");
					...
					ReportParams reportParams = new ReportParams();
					reportParams.setBaseColumn("sum(Gross_Profit) as Profit,sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
					reportParams.setCompanyId(currentUser.getCompany().getCompanyId());
					reportParams.setEndDate(endDat);
					reportParams.setGroupBy("Product");
					reportParams.setMainBaseColumn("Product");
					reportParams.setOrderBy("");
					reportParams.setPivotColumn( "CREATED_DATE");
					reportParams.setPrintColumns( reportType+","+ "Revenue,Cost of Goods,Margin,Tax,Avg Discount");
					reportParams.setReportDateType(reportDateType);
					reportParams.setStartDate(startDat);
					reportParams.setTableName("SaleReport_withSale");
					...
					ReportParams reportParams = new ReportParams();
					reportParams.setBaseColumn("sum(Items_Sold),sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,sum(Gross_Profit) as Profit,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
					reportParams.setCompanyId(currentUser.getCompany().getCompanyId());
					reportParams.setEndDate(endDat);
					reportParams.setGroupBy("Product");
					reportParams.setMainBaseColumn("Product");
					reportParams.setOrderBy("");
					reportParams.setPivotColumn( "CREATED_DATE");
					reportParams.setPrintColumns( reportType+","+ "Revenue,Cost of Goods,Gross Profit,Margin,Tax,Avg Discount");
					reportParams.setReportDateType(reportDateType);
					reportParams.setStartDate(startDat);
					reportParams.setTableName("SaleReport_withSale");
					...
					ReportParams reportParams = new ReportParams();
					reportParams.setBaseColumn("avg(Margin) as Margin,sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,sum(Gross_Profit) as Profit,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
					reportParams.setCompanyId(currentUser.getCompany().getCompanyId());
					reportParams.setEndDate(endDat);
					reportParams.setGroupBy("Product");
					reportParams.setMainBaseColumn("Product");
					reportParams.setOrderBy("");
					reportParams.setPivotColumn( "CREATED_DATE");
					reportParams.setPrintColumns( reportType+","+ "Revenue,Cost of Goods,Gross Profit,Tax,Avg Discount");
					reportParams.setReportDateType(reportDateType);
					reportParams.setStartDate(startDat);
					reportParams.setTableName("SaleReport_withSale");
					...
					ReportParams reportParams = new ReportParams();
					if(!isHeadOffice && configuration!=null && configuration.getPropertyValue().toString().equalsIgnoreCase(ControllersConstants.TRUE)){
						reportParams.setBaseColumn("sum(Revenue_tax_incl) as Revenue_Tax_Incl,sum(Revenue) as Revenue,Tax,Avg Discount");
						reportParams.setPrintColumns( reportType+","+ "Revenue,Tax");
						
					}else{
						reportParams.setBaseColumn("sum(Revenue_tax_incl) as Revenue_Tax_Incl,sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,sum(Gross_Profit) as Profit,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
						reportParams.setPrintColumns( reportType+","+ "Revenue,Cost of Goods,Gross Profit,Margin,Tax,Avg Discount");
					...
					ReportParams reportParams = new ReportParams();
					if(!isHeadOffice && configuration!=null && configuration.getPropertyValue().toString().equalsIgnoreCase(ControllersConstants.TRUE)){
						reportParams.setBaseColumn("min(CREATED_DATE) as First,sum(Revenue) as Revenue,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
						reportParams.setPrintColumns( reportType+","+ "Revenue,Tax,Avg Discount");
							
					}else{
						reportParams.setBaseColumn("min(CREATED_DATE) as First,sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,sum(Gross_Profit) as Profit,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
						reportParams.setPrintColumns( reportType+","+ "Revenue,Cost of Goods,Gross Profit,Margin,Tax,Avg Discount");
					...
					ReportParams reportParams = new ReportParams();
					if(!isHeadOffice && configuration!=null && configuration.getPropertyValue().toString().equalsIgnoreCase(ControllersConstants.TRUE)){
						reportParams.setBaseColumn("max(CREATED_DATE) as Last,sum(Revenue) as Revenue,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
						reportParams.setPrintColumns(  reportType+","+ "Revenue,Tax,Avg Discount");
								
					}else{
						reportParams.setBaseColumn("max(CREATED_DATE) as Last,sum(Revenue) as Revenue,sum(Cost_of_Goods) as Cost,sum(Gross_Profit) as Profit,avg(Margin) as Margin,Tax,avg(ITEM_DISCOUNT_PRCT) as Discount");
						reportParams.setPrintColumns(  reportType+","+ "Revenue,Cost of Goods,Gross Profit,Margin,Tax,Avg Discount");
