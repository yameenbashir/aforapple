 ...
 */
@Controller
@RequestMapping("/userSalesReport")
public class UserSalesReportController {
	...
	@RequestMapping(value = "/getUserSalesReportByDateRange/{sessionId}/{startDate}/{endDate}/{reportType}/{reportDateType}/{outletName}", method = RequestMethod.POST)
	public @ResponseBody Response getUserSalesReportByDateRange(@PathVariable("sessionId") String sessionId,
		...
		if(SessionValidator.isSessionValid(sessionId, request)){
			...
			try {
				...
				if(reportType.equals("Revenue")){
					...
					reportParams.setGroupBy("User");
					reportParams.setMainBaseColumn("User");
					...
					reportParams.setTableName("User_Sales_Report");
				...
				if (tableData != null) {
					...
					util.AuditTrail(request, currentUser, "UserSalesReportController.getUserSalesReportByDateRange", 
							"User "+ currentUser.getUserEmail()+" retrived user sales report against date range successfully ",false);
		...
		if(SessionValidator.isSessionValid(sessionId, request)){
			...
			try {
				...
				if(outlets!=null){
					...
					util.AuditTrail(request, currentUser, "UserSalesReportController.getOutlets", 
