...
/**
 * NewCompositeProductController.js
 * @constructor
 */
var NewCompositeProductController = ['$scope', '$http', '$window','$cookieStore','$rootScope','$timeout','$route','$sce','SessionService','NewCompositeProductControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,$timeout,$route,$sce,SessionService,NewCompositeProductControllerPreLoad) {
	...
	$scope.compositProduct.compositeQunatity ="1";
	$scope.tagList = [];
	...
	$scope.calculateCompositeProductQuantity = function(){
		if($scope.gui.compositePrduct && $scope.productList.length>0){
			$scope.calculateProductSupplierPrice();
		}
	};
	
	$scope.addCompositeProductDynamically = function(){
		$scope.dynamicError = false;
		$scope.gui.compositeQunatity = 0;
		if(!$scope.gui.varientProducts){
			if(!isNaN($scope.outletList[0].currentInventory) && $scope.outletList[0].currentInventory!= null
					&& $scope.outletList[0].currentInventory !='null' && $scope.outletList[0].currentInventory!=''){
				$scope.gui.compositeQunatity = parseInt($scope.outletList[0].currentInventory);
			}
		}else{
			if($scope.gui.varientProducts && $scope.gui.trackingProduct && $scope.productVariantValuesCollection.length>0){
				for(var i=0;i<$scope.productVariantValuesCollection.length;i++){
					for(var j=0;j<$scope.productVariantValuesCollection[i].varientsOutletList.length;j++){
						if(!isNaN($scope.productVariantValuesCollection[i].varientsOutletList[j].currentInventory) && $scope.productVariantValuesCollection[i].varientsOutletList[j].currentInventory!= null
								&& $scope.productVariantValuesCollection[i].varientsOutletList[j].currentInventory !='null' && $scope.productVariantValuesCollection[i].varientsOutletList[j].currentInventory!=''){
							$scope.gui.compositeQunatity = $scope.gui.compositeQunatity+parseInt($scope.productVariantValuesCollection[i].varientsOutletList[j].currentInventory);
						}
					}
				}
			}
		...
		if($scope.productVariantBean.isProduct.toString() == "true"){
			...
			if(!checkproductVariantExistinProductList($scope.productVariantBean.productId) ){
				...
				$scope.dynamicError = true;
				$scope.dynamicErrorMessage = "Product "+$scope.productVariantBean.variantAttributeName+" already added.";
				$timeout(function(){
					$scope.dynamicError = false;
				}, 2000);
			}
				...
				if($scope.productVariantBean.productVariantId == $scope.productVariantBeanList[i].productVariantId){
					...
					if(!checkproductVariantExistinProductList($scope.productVariantBeanList[i].productVariantId)){
						...
						if($scope.gui.isConsumeCompositeQunatity && parseInt($scope.temp.uniteQunatity)<=parseInt($scope.temp.currentInventory)
							...
							$scope.dynamicError = true;
							$scope.dynamicErrorMessage = "Product Variant "+$scope.productVariantBean.variantAttributeName+" cousume quantity can not be great than available quantity."+$scope.productVariantBean.currentInventory;
							$timeout(function(){
								$scope.dynamicError = false;
							}, 2000);
						}
						...
						$scope.dynamicError = true;
						$scope.dynamicErrorMessage = "Product Variant "+$scope.productVariantBean.variantAttributeName+" already added.";
						$timeout(function(){
							$scope.dynamicError = false;
						}, 2000);
					}
	...
	$scope.removeProduct = function(product){
		if($scope.productList.length>0){
			for(var i=0;i<$scope.productList.length;i++){
				if($scope.productList[i].productVariantId==product.productVariantId){
			...
			for(var i=0;i<$scope.productList.length;i++){
				if(!isNaN($scope.productList[i].uniteQunatity)){
					if($scope.gui.isConsumeCompositeQunatity && parseInt($scope.productList[i].uniteQunatity)<=parseInt($scope.productList[i].currentInventory)
						...
						$scope.dynamicError = true;
						$scope.dynamicErrorMessage = "Product/Variant "+$scope.productList[i].variantAttributeName+" cousume quantity can not be great than available quantity..";
						$timeout(function(){
							$scope.dynamicError = false;
						}, 2000);
						$scope.productList[i].compositeQunatityConsumed = "0";
						var retailPric = parseFloat($scope.productList[i].retailPrice)*parseFloat($scope.productList[i].compositeQunatityConsumed);
						retailPrice = retailPrice+retailPric;
						//$scope.productList[i].compositeQunatity = $scope.productList[i].currentInventory;
					}
				}
			}
		}
	...
	$scope.addCompositProductDynamically_old = function(){
		...
		if(!isExist){
			...
			$scope.compositProductTemp.compositeQunatity = $scope.compositProduct.compositeQunatity;
	...
	$scope.sesssionValidation = function(){
		...
		if(SessionService.validate()){
			...
			$scope.data = NewCompositeProductControllerPreLoad.loadControllerData();
			$scope.fetchData();
	...
	$scope.gui.compositePrduct = false;
	//$scope.allowSale = true; 
	$scope.trackInventory = true;
	$scope.mutltiProduct = function(){
		if($scope.gui.compositePrduct){
			$scope.gui.compositePrduct = false;

		}else{
			$scope.gui.compositePrduct = true;
		}
	...
	$scope.addProduct = function(dataUrl, name) {
		...
		if($scope.gui.compositePrduct && $scope.productList.length==0){
			$scope.productAddError = true;
			$scope.productAddErrorMessage = "Please add atleast one product to create composite product.";
			$timeout(function(){
				$scope.productAddError = false;
			}, 1000);
			return;
		...
		$scope.productBean.isComposite = $scope.gui.compositePrduct;
		...
		$scope.productBean.productList = $scope.productList;
		$scope.setDefaultDutyCalcuation();
		$scope.loading = true;
		$http.post('newCompositeProduct/addProduct/'+$scope._s_tk_com, $scope.productBean)
		.success(function(Response) {
			$scope.responseStatus = Response.status;
			if ($scope.responseStatus == 'SUCCESSFUL') {
				$scope.productSuccess = true;
	...
	$scope.upload = function (dataUrl, name) {
	    ...
	    $scope.autoCompleteOptions = {
				...
				data : function(term) {
					...
					var productBeansResults = _.filter($scope.productBeansList, function(val) {
						return val.variantAttributeName.toLowerCase().includes(term) || val.sku.toLowerCase().includes(term);

					});
					var list = productVariantResults.concat(productBeansResults);
					return list;
				},
				renderItem : function(item) {
