...
/**
 * ManageCompositeProductController.js
 * @constructor
 */
var ManageCompositeProductController = ['$scope', '$http', '$window','$cookieStore','$rootScope','$timeout','$route','$sce','SessionService','ManageCompositeProductControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,$timeout,$route,$sce,SessionService,ManageCompositeProductControllerPreLoad) {
	...
	$scope.fetchData = function(){
		$scope.data = ManageCompositeProductControllerPreLoad.loadControllerData();
		...
		if($scope.data == 'NORECORDFOUND'){
			...
			if($scope.data!=null){
				...
				if($scope.data.productBean!=null){
					...
					if($scope.productBean.isComposite=="true"){
						$scope.gui.compositePrduct = true;
					}else{
...
//				$scope.productBean.outletId = $scope.outletList[i].outletId;
	...
	$scope.updateProduct = function(dataUrl, name) {
		...
		$scope.setDefaultDutyCalcuation();
		$scope.loading = true;
		$http.post('manageCompositeProduct/updateCompositeProduct/'+$scope._s_tk_com, $scope.productBean)
		.success(function(Response) {
			$scope.loading = false;
